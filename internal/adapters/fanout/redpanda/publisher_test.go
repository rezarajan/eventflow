...
func TestPublisherOpenValidatesSingleTopic(t *testing.T) {
	...
	publisher := NewWithDependencies(Config{Brokers: []string{"broker:9092"}, Topic: "datascape.events.v1", TopicMode: "single"}, &fakeProducerFactory{}, validator)
	if err := publisher.Open(context.Background()); err != nil {
		t.Fatalf("Open returned error: %v", err)
	}
	if got := validator.calls[0]; got != "datascape.events.v1" {
		t.Fatalf("validated topic = %q, want datascape.events.v1", got)
	}
}
...
func TestPublisherOpenRequiresBrokers(t *testing.T) {
	publisher := NewWithDependencies(Config{Topic: "datascape.events.v1", TopicMode: "single"}, &fakeProducerFactory{}, &fakeTopicValidator{})
...
func TestPublisherRequiresOpenBeforePublish(t *testing.T) {
	publisher := NewWithDependencies(Config{Brokers: []string{"broker:9092"}, Topic: "datascape.events.v1", TopicMode: "single"}, &fakeProducerFactory{}, &fakeTopicValidator{})
...
func TestPublisherPublishWritesSynchronousMessage(t *testing.T) {
	...
	publisher := NewWithDependencies(Config{Brokers: []string{"broker:9092"}, Topic: "datascape.events.v1", TopicMode: "single"}, factory, &fakeTopicValidator{})
	if err := publisher.Open(context.Background()); err != nil {
		t.Fatalf("Open returned error: %v", err)
	}
...
func TestPublisherTypePrefixValidatesDynamicTopic(t *testing.T) {
	...
	if err := publisher.Publish(context.Background(), redpandaTestEvent(t, "1", "attendance.submitted.v1")); err != nil {
		t.Fatalf("Publish returned error: %v", err)
	}
	if err := publisher.Publish(context.Background(), redpandaTestEvent(t, "2", "attendance.submitted.v1")); err != nil {
		t.Fatalf("Publish returned error: %v", err)
	}
	if len(validator.calls) != 1 || validator.calls[0] != "attendance.events.v1" {
...
func TestPublisherCloseClosesProducers(t *testing.T) {
	...
	if !factory.producers["datascape.events.v1"].closed {
